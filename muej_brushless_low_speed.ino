...
// fast
//const int pwmSin[] = {127, 147, 166, 185, 202, 217, 230, 240, 248, 252, 254, 252, 248, 240, 230, 217, 202, 185, 166, 147, 127, 107, 88, 70, 53, 37, 24, 14, 6, 2, 0, 2, 6, 14, 24, 37, 52, 69, 87, 107, 127};

// SVPWM (Space Vector Wave) best for Ramon Motor
...
void loop() {
  ...
  analogWrite(IN1, pwmSin[currentStepA]*power);
  analogWrite(IN2, pwmSin[currentStepB]*power);
  analogWrite(IN3, pwmSin[currentStepC]*power);
  potState = analogRead(A0);
  //potState = 300;
  if (potState > 612)
  {
    direct = false;
    potState = map(potState, 612, 1024, 1000, 10);
  }
  else
  {
    direct = true;
    potState = map(potState, 0, 611, 10, 1000);
  }
